...
var readBoards = function (directory) {
  return readDir(directory)
    ...
    .then(boards => Promise.all(boards.map(board => readFile(board))))
    .then(boards => boards.map(board => JSON.parse(board)));
};
  ...
  return readBoards(directory)
    .then(boards => boards
      ...
      .forEach(board => {
        ...
        console.log();
        board.cards.forEach((card, i) => console.log((i + 1) + '. ' + card.name));
      })
    );
};
